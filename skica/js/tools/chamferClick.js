...
import { findObjectAt, calculateAllIntersections, chamferTwoLines } from '../geometry.js';
import { showChamferDialog } from '../dialogs.js';
import { getLineSegment, analyzeSelection } from './helpers.js';
import { isAnchored } from './anchorClick.js';
import { updateAssociativeDimensions } from '../dialogs/dimension.js';

/** Klik na první úsečku → dialog → klik na druhou → zkosení. */
export function handleChamferClick(wx, wy) {
  const idx = findObjectAt(wx, wy);
  if (idx === null) { showToast("Klepněte na první úsečku pro zkosení"); return; }
  const obj1 = state.objects[idx];
  const ls1 = getLineSegment(obj1, wx, wy);
  if (!ls1) {
    showToast("Zkosení funguje pouze pro úsečky a rovné segmenty kontur");
    return;
  }

  state.selected = idx;
  renderAll();
  setHint("Zadejte parametry zkosení");

  showChamferDialog((dist1, dist2) => {
    ...
    function onSecondClick(e) {
      ...
      if (!ls2) { showToast("Zkosení funguje pouze pro úsečky a rovné segmenty kontur"); return; }
      ...
      pushUndo();
      const result = chamferTwoLines(proxy1, proxy2, dist1, dist2);
      ...
      result.line.name = `Zkosení ${dist1}×${dist2}`;
      addObject(result.line);
      calculateAllIntersections();
      updateAssociativeDimensions();
      renderAll();
      resetHint();
      showToast(`Zkosení ${dist1}×${dist2} vytvořeno ✓`);
    }
...
/** Zkosení z 2 předvybraných úseček. Vrací true pokud se operace provedla. */
export function chamferFromSelection() {
  ...
  if (obj1.type === 'polyline' && info1.segIdx !== null) {
    ...
    if ((obj1.bulges?.[si] || 0) !== 0) { showToast("Zkosení obloukového segmentu není podporováno"); return true; }
  ...
  if (obj2.type === 'polyline' && info2.segIdx !== null) {
    ...
    const p1 = v[si], p2 = v[(si + 1) % n];
    if ((obj2.bulges?.[si] || 0) !== 0) { showToast("Zkosení obloukového segmentu není podporováno"); return true; }
