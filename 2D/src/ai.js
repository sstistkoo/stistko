...
window.enableAIDragging = function () {
  ...
  try {
    ...
    if (saved) {
      ...
      panel.style.right = 'auto';
      panel.style.transform = 'none'; // Důležité: zrušit translateX(-50%)
    }
  } catch (e) {}
  ...
  const onStart = (clientX, clientY) => {
    ...
    // Při začátku přetahování zrušit transform pro přesné pozicování
  ...
  const onMove = (clientX, clientY) => {
    ...
    panel.style.right = 'auto';
    panel.style.transform = 'none'; // Zajistit že transform je vypnutý
  };
  ...
  const onEnd = () => {
    ...
    try {
      localStorage.setItem('aiPanelPosition', JSON.stringify({
        ...
        top: panel.style.top,
        transform: 'none' // Uložit info že panel byl přesunut
      }));
    } catch (e) {}
  };

  // Kontrola zda cíl není interaktivní prvek
  ...
  header.addEventListener('mousedown', (e) => {
    if (isInteractive(e.target)) return; // Nekreslit při kliknutí na interaktivní prvky
...
// Podporuje dynamické načítání free modelů z AI modulu
window.updateModelsForProvider = function() {
  ...
  // V Auto módu - použij loadAutoModeModels
  ...
  // V Manual módu - načti FREE modely z API
  ...
  // Získej modely z AI modulu pokud je dostupný
  if (typeof AI !== 'undefined' && AI.ALL_MODELS && AI.ALL_MODELS[provider]) {
    const aiModels = AI.ALL_MODELS[provider];
    let firstEnabled = null;

    // Přidej "Načítám..." placeholder pokud budeme dynamicky načítat
    if (aiModels.length === 0) {
      const loadingOpt = document.createElement("option");
      loadingOpt.value = "";
      loadingOpt.textContent = "⏳ Načítám modely...";
    ...
    // Fallback na statické modely pokud AI modul není dostupný
...
// Načti dynamické free modely z AI modulu/API
window.loadDynamicModels = async function(provider, modelSelect) {
  ...
  try {
    if (typeof AI !== 'undefined') {
      // Získej všechny modely pro providera
      ...
      if (freeModels.length > 0) {
        ...
        headerOpt.textContent = `═══ FREE MODELY (${freeModels.length}) ═══`;
        ...
        freeModels.forEach((model, idx) => {
          const option = document.createElement("option");
          option.value = model.value;
          const qualityIcon = model.quality >= 90 ? '🏆' : model.quality >= 70 ? '⭐' : '⚡';
    ...
    console.error('Chyba při načítání dynamických modelů:', err);
    modelSelect.innerHTML = '<option value="" disabled>❌ Chyba načítání</option>';
  }
...
// ═══════════════════════════════════════════════════════════════════════════════
// PROCESS AI RESPONSE - Zpracuje odpověď z AI modulu
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Zpracuje AI odpověď (z AI modulu nebo původní implementace)
 * Parsuje JSON, přidá tvary na canvas, zobrazí v chatu
 */
window.processAIResponse = async function(aiResponseText, originalPrompt) {
  ...
  try {
    ...
    // Ulož raw odpověď pro debugging
...
// Nová funkce využívající sdílený AI modul pro lepší správu modelů a fallback
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Zavolá AI pomocí sdíleného AI Module v3.0
 * Podporuje auto-fallback, smart retry a rate limit management
 * @param {string} prompt - Uživatelský dotaz
 * @param {object} options - Volitelné nastavení
 * @returns {Promise<string>} AI odpověď
 */
window.callAIWithModule = async function(prompt, options = {}) {
  // Zkontroluj zda je AI modul načten
  if (typeof AI === 'undefined') {
    console.warn('⚠️ AI modul není načten, používám původní metodu');
    return null; // Vrátí null = použij původní metodu
  }

  // Zjisti aktuální nastavení z UI
  const providerSelect = document.getElementById("aiProviderSelect");
  const modelSelect = document.getElementById("aiModelSelect");
  ...
  // Pokud je auto-switch zapnutý a máme Smart AI State
  ...
  if (!systemPrompt) {
    if (aiType === 'chat') {
      systemPrompt = 'Jsi přátelský AI asistent. Odpovídej v češtině.';
    } else {
      // Pro CNC/2D použij původní komplexní system prompt
      systemPrompt = window.buildCADSystemPrompt ? window.buildCADSystemPrompt() : null;
    }
  }

  try {
    // Použij AI.ask() nebo AI.smartAsk() pro automatický fallback
    ...
    console.log(`✅ AI Module: Odpověď od ${provider}/${model}`);
    return response;
...
/**
 * Wrapper pro callGemini který zkusí nejdřív AI modul
 * Pokud selže nebo není dostupný, použije původní implementaci
 */
window.callGeminiWithFallback = async function() {
  ...
  try {
    ...
    if (moduleResponse !== null) {
      // AI modul vrátil odpověď, zpracuj ji
      await window.processAIResponse(moduleResponse, prompt);
      return;
    }
  } catch (err) {
    console.warn('⚠️ AI modul selhal, zkouším původní metodu:', err.message);
  }
 ...
 * Zpracuje prompt, odešle na API a zpracuje odpověď.
 * @async
 * @returns {Promise<void>}
 */
