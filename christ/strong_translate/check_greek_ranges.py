...
print(
    f"Greek S En: {len(greek_with_en)} entries, min: G{min(greek_with_en)}, max: G{max(greek_with_en)}"
)
print(f"\nGreek BEZ En: {len(greek_without_en)} entries")
print(f"  Range: G{min(greek_without_en)} - G{max(greek_without_en)}")
print(
    f"  Continuous: {'YES' if len(greek_without_en) > 1 and greek_without_en[-1] - greek_without_en[0] == len(greek_without_en) - 1 else 'NO'}"
)
